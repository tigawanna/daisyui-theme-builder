...
export function DaisyUiDevtools({
  drawer_id = "daisyui-devtools-drawer",
}: DaisyUiDevtoolsProps) {
  const [searchParams] = useState(defaultThemes({}));  
  console.log("drawer id ", drawer_id);
  return (
    <>
      <label
        htmlFor="daisyui-devtools-drawer"
        className="btn drawer-button fixed top-[10%] right-[5%] flex gap-2 rounded-full z-50 text-primary"
      >
        <svg
         className="w-8 h-8 text-primary fill-primary hover:fill-accent"
      ...
      <div className="drawer drawer-end sticky  z-50">
