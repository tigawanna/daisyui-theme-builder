/* prettier-ignore-start */

/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file is auto-generated by TanStack Router

import { createFileRoute } from '@tanstack/react-router'

// Import Routes

import { Route as rootRoute } from './routes/__root'
import { Route as IndexImport } from './routes/index'
import { Route as TwarkuiIndexImport } from './routes/twarkui/index'
import { Route as ShadcnIndexImport } from './routes/shadcn/index'

// Create Virtual Routes

const DaisyuiIndexLazyImport = createFileRoute('/daisyui/')()

// Create/Update Routes

const IndexRoute = IndexImport.update({
  path: '/',
  getParentRoute: () => rootRoute,
} as any)

const DaisyuiIndexLazyRoute = DaisyuiIndexLazyImport.update({
  path: '/daisyui/',
  getParentRoute: () => rootRoute,
} as any).lazy(() => import('./routes/daisyui/index.lazy').then((d) => d.Route))

const TwarkuiIndexRoute = TwarkuiIndexImport.update({
  path: '/twarkui/',
  getParentRoute: () => rootRoute,
} as any)

const ShadcnIndexRoute = ShadcnIndexImport.update({
  path: '/shadcn/',
  getParentRoute: () => rootRoute,
} as any)

// Populate the FileRoutesByPath interface

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexImport
      parentRoute: typeof rootRoute
    }
    '/shadcn/': {
      id: '/shadcn/'
      path: '/shadcn'
      fullPath: '/shadcn'
      preLoaderRoute: typeof ShadcnIndexImport
      parentRoute: typeof rootRoute
    }
    '/twarkui/': {
      id: '/twarkui/'
      path: '/twarkui'
      fullPath: '/twarkui'
      preLoaderRoute: typeof TwarkuiIndexImport
      parentRoute: typeof rootRoute
    }
    '/daisyui/': {
      id: '/daisyui/'
      path: '/daisyui'
      fullPath: '/daisyui'
      preLoaderRoute: typeof DaisyuiIndexLazyImport
      parentRoute: typeof rootRoute
    }
  }
}

// Create and export the route tree

export const routeTree = rootRoute.addChildren({
  IndexRoute,
  ShadcnIndexRoute,
  TwarkuiIndexRoute,
  DaisyuiIndexLazyRoute,
})

/* prettier-ignore-end */

/* ROUTE_MANIFEST_START
{
  "routes": {
    "__root__": {
      "filePath": "__root.tsx",
      "children": [
        "/",
        "/shadcn/",
        "/twarkui/",
        "/daisyui/"
      ]
    },
    "/": {
      "filePath": "index.tsx"
    },
    "/shadcn/": {
      "filePath": "shadcn/index.tsx"
    },
    "/twarkui/": {
      "filePath": "twarkui/index.tsx"
    },
    "/daisyui/": {
      "filePath": "daisyui/index.lazy.tsx"
    }
  }
}
ROUTE_MANIFEST_END */
